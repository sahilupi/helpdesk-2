...
function* getSuggestions({ payload }) {
  yield put({ type: FETCH_COMMENT_SUGGESTIONS.PENDING })
  try {
    const response = yield call(api.tickets.getSuggestions, payload)
    yield put({ type: FETCH_COMMENT_SUGGESTIONS.SUCCESS, response })
  } catch (error) {
    const errorText = error.response ? error.response.data.error : error
    if (error.response && error.response.status !== (401 || 403)) {
      Log.error(errorText, error)
    ...
    yield put({ type: FETCH_COMMENT_SUGGESTIONS.ERROR, error })
  }
}
...
export default function* watcher() {
  ...
  yield takeLatest(FETCH_COMMENT_SUGGESTIONS.ACTION, getSuggestions)
